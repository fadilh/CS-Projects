...
public class MyHashTableTester {
	...
	public void test01_putWithPreviousValue() {
		assertTrue("Replacing Mary should return previously mapped value (80)",
				table.put(new Person("Mary"), 50) == 80); // return prev. value
	...
	public void test04_getWithCollision() {
		...
		assertTrue("Your Person.java needs to use the String class' hashCode method",
				"Siblings".hashCode() == new Person("Siblings").hashCode());
	...
	public void test07_removeExistsWithCollisionFirstElementFirst() {
		...
		assertTrue("Removing first item in collision list should return 60",
				table.remove(new Person("Siblings")) == 60);
	...
	public void test08_removeExistsWithCollisionSecondElementFirst() {
		...
		assertTrue("Removing second item in collision list should return 70",
				table.remove(new Person("Teheran")) == 70);
	...
	/*
	 * This test may take some time to run, uncomment if you want to check the
	 * performance
	 * of your hash table. Note that it will require an extra method be written to
	 * work
	 */
	// @Test
	...
	//
	// System.out.println("table lookup: " + (System.currentTimeMillis() - start) +
	// "ms");
	//
	// List<Person> list = new ArrayList<>(size);
	//
	// //you will need to implement the keySet method (similar to java.util.HashMap)
	// list.addAll(table.keySet()); //keySet should return a Set<K>, where K is a
	// Person in this case
	//
	...
	//
	// System.out.println("list contains: " + (System.currentTimeMillis() - start) +
	// "ms");
	// }
}
